// Simple get_opt replacement class (C++11).
...
// }
//
// 2) Simple OOP map-based api `get_opt class`. Initialization (argc, argv) pair required.
//
//    This get_opt class is a std::map replacement where key/value are std::string types.
...
#define GET_OPT_VERSION "1.0.0" // (2016/04/18) Initial version

namespace get_opt_utils
{
    // string conversion
    ...
    #       else
            ...
            char f_name[32] = {};
            sprintf(f_name, "/proc/%d/cmdline", pid);
...
struct get_opt : public std::map< std::string, std::string >
{
    ...
    get_opt(int argc, const char **argv ) : super() {
        ...
        for( auto &it : args ) {
            ...
            auto size = get_opt_utils::split(tokens, it, "=" );
...
template< typename T >
inline T get_arg(const T &defaults, const char *argv ) {
    static struct get_opt map(get_opt_utils::cmdline() );
    return map.has( argv ) ? get_opt_utils::as<T>(map[ argv ]) : defaults;
}

template< typename T, typename... Args >
inline T get_arg(const T &defaults, const char *arg0, Args... argv ) {
...
inline const char * get_arg(const char *defaults, const char *argv ) {
    static struct get_opt map(get_opt_utils::cmdline() );
    return map.has( argv ) ? get_opt_utils::as<const char *>(map[ argv ]) : defaults;
}

template< typename... Args >
inline const char * get_arg(const char *defaults, const char *arg0, Args... argv ) {
